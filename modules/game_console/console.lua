...
local function parseColorTags(text)
  local result = {}
  local defaultColor = "#FFFFFF"
  local lastPos = 1
  while true do
    local startIndex, endIndex, colorCode = text:find('<color="(#[0-9A-Fa-f]+)">', lastPos)
    if not startIndex then
      local remainder = text:sub(lastPos)
      if remainder ~= "" then
    ...
    if startIndex > lastPos then
      local before = text:sub(lastPos, startIndex - 1)
      table.insert(result, before)
      table.insert(result, defaultColor)
    end
    local closeTagStart, closeTagEnd = text:find('</color>', endIndex + 1)
    if not closeTagStart then
      local coloredText = text:sub(endIndex + 1)
      table.insert(result, coloredText)
      table.insert(result, colorCode)
      break
    end
    local coloredText = text:sub(endIndex + 1, closeTagStart - 1)
    table.insert(result, coloredText)
    table.insert(result, colorCode)
    lastPos = closeTagEnd + 1
  end
  return result
end
...
function addTabText(text, speaktype, tab, creatureName)
  ...
  if text:find('<color="') then
    local parsedTable = parseColorTags(text)
    if #parsedTable >= 2 then
      label:setColoredText(parsedTable)
    else
      label:setText(text)
