...
vec4 EmbossFull(sampler2D txt,vec2 uv,float angle,vec4 color)
{
 ...
 float n=abs(sin(perspective.x*35.)+cos(perspective.y*4.+u_Time*2.));
 float i=pow(n,2.);
 noise.rgb=vec3(i);
 noise=noise*6.;
 noise.a=1.;
...
void main(void)
{
 ...
 vec4 _color=vec4(0.1333, 0.1529, 0.7255, 1.0);
 ...
 float _ShadowLight_Size_1=.15;//("_ShadowLight_Size_1",Range(0,16))=.686
 float _ShadowLight_Intensity_1=.3;//("_ShadowLight_Intensity_1",Range(0,4))=3.664
