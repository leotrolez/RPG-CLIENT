...
vec4 EmbossFull(sampler2D txt,vec2 uv,float angle,vec4 color)
{
 ...
 metal=mod(metal,1.);
 metal=smoothstep(.45,.55,metal);// PUT LESS TO MAKE MORE ELETRIC
 
 vec4 noise=vec4(0.,0.,0.,0.);
 float n=abs(sin(perspective.x*140.)+cos(uv.y*16.+u_Time*2.));
 float i=pow(n,2.);
 noise.rgb=vec3(i);
 noise=noise*6.;
 noise.a=1.;
...
void main(void)
{
 ...
 vec4 _color=vec4(0.0157, 0.2745, 0.1882, 1.0);
