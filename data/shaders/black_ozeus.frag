...
vec4 EmbossFull(sampler2D txt,vec2 uv,float angle,vec4 color)
{
 vec4 txt1=texture2D(txt,uv);
 uv*=2.5;
 float time=(u_Time/4.);
 float a=time*1.;
 vec2 perspective=rotate(uv,(angle/180.)*3.14);
 float metal=length(vec2(sin(a+2.*perspective.x),sin(a-200.*perspective.x)+sin(a+2.*perspective.y)+sin(a+5.*perspective.y)));
 metal+=txt1.r*.21+txt1.g*.4+txt1.b*.2;
 metal=mod(metal,1.);
 metal=smoothstep(.0015,.15,metal);// PUT LESS TO MAKE MORE ELETRIC
 
 vec4 noise=vec4(0.,0.,0.,0.);
 float n=abs(sin(perspective.x*140.)+cos(perspective.y*16.+u_Time*2.));
 float i=pow(n,2.);
 noise.rgb=vec3(i);
 noise=noise*6.;
 noise.a=1.;
...
void main(void)
{
 ...
 float _EmbossFull_Angle_1=315.;// ("_EmbossFull_Angle_1",Range(-1,1))=0
 float _SpriteFade=1.;// ("SpriteFade",Range(0,1))=1.
 vec4 _color=vec4(0.0588, 0.0078, 0.0078, 1.0);
 
 vec4 _EmbossFull_1=EmbossFull(u_Tex0,v_TexCoord,_EmbossFull_Angle_1,_color);
 ...
 float _ShadowLight_Size_1=.10;//("_ShadowLight_Size_1",Range(0,16))=.686
 float _ShadowLight_Intensity_1=.9;//("_ShadowLight_Intensity_1",Range(0,4))=3.664
