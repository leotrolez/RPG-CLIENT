...
void main(void)
{
 ...
 vec2 UV=v_TexCoord;
 vec2 TEXTURE_PIXEL_SIZE=vec2(.0015);// CHANGE THIS
 
 vec3 c;
 vec4 con0,con1,con2,con3;
 
 // "rendersize" refers to size of source image before upscaling.
 vec2 rendersize=vec2(1.)/TEXTURE_PIXEL_SIZE;
 vec2 iResolution=vec2(1.)/TEXTURE_PIXEL_SIZE;//TODO
 FsrEasuCon(
 ...
 vec3 bC=texture2D(u_Tex0,(p0+off.xw)).rgb;float bL=bC.g+.5*(bC.r+bC.b);
 vec3 cC=texture2D(u_Tex0,(p0+off.yw)).rgb;float cL=cC.g+.5*(cC.r+cC.b);
 vec3 iC=texture2D(u_Tex0,(p1+off.xw)).rgb;float iL=iC.g+.5*(iC.r+iC.b);
 vec3 jC=texture2D(u_Tex0,(p1+off.yw)).rgb;float jL=jC.g+.5*(jC.r+jC.b);
 vec3 fC=texture2D(u_Tex0,(p1+off.yz)).rgb;float fL=fC.g+.5*(fC.r+fC.b);
 vec3 eC=texture2D(u_Tex0,(p1+off.xz)).rgb;float eL=eC.g+.5*(eC.r+eC.b);
 vec3 kC=texture2D(u_Tex0,(p2+off.xw)).rgb;float kL=kC.g+.5*(kC.r+kC.b);
 vec3 lC=texture2D(u_Tex0,(p2+off.yw)).rgb;float lL=lC.g+.5*(lC.r+lC.b);
 vec3 hC=texture2D(u_Tex0,(p2+off.yz)).rgb;float hL=hC.g+.5*(hC.r+hC.b);
 vec3 gC=texture2D(u_Tex0,(p2+off.xz)).rgb;float gL=gC.g+.5*(gC.r+gC.b);
 vec3 oC=texture2D(u_Tex0,(p3+off.yz)).rgb;float oL=oC.g+.5*(oC.r+oC.b);
 vec3 nC=texture2D(u_Tex0,(p3+off.xz)).rgb;float nL=nC.g+.5*(nC.r+nC.b);
